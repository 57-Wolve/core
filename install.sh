...
echo 'alias hydradyne="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "git@github.com:57-Wolve/core.git" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'git@github.com:57-Wolve/core.git'"
...
rm -rf "$HOME/.config/@hydradyne"
rm -rf "$HOME/.config/@hydradyne"
rm -rf "$HOME/.config/hydradyne-core"
